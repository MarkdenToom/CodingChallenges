...
1.  create 'zclip.bat' in 'C:\Windows' with the following line:
    @pyw.exe "[insert the directory you saved 9 Extending the Multi-Clipboard.pyw in]" %*
...
Remove and removeall keys from the dictionary."""
...
if len(sys.argv) == 3:
    if sys.argv[1].lower() == 'save':  # if we want to save a clipboard, then do:
        mcbShelf[sys.argv[2]] = pyperclip.paste()  # set keyword as key for mcbShelf and paste to clipboard
    if sys.argv[1].lower() == 'delete':  # if we want to delete a key:
        # using pop() instead of del function in order to avoid errors
        mcbShelf.pop(sys.argv[2], None)  # remove key from dictionary, if not present; return None
    ...
    elif sys.argv[1].lower() == 'deleteall':  # if requesting a deletion of all keys, do:
        mcbShelf.clear()  # remove all saved keys and their respective values
